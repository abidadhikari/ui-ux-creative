...
export default function NavBar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 pt-10  w-full flex   justify-center items-center ">
      <button
        type="button"
        className=" sm:hidden shadow-2xl rounded-full cursor-pointer backdrop-blur-2xl text-black p-2 fixed right-[20px] top-[20px] border 300 z-50 "
        onClick={() => {
      ...
      {openNavBar && (
        <nav className=" flex  items-center justify-around sm:justify-center flex-col sm:flex-row gap-0  text-sm md:text-lg text-black bg-white backdrop-blur-2xl w-[90vw] sm:w-fit sm:max-w-[90%] px-[10px] md:px-[45px] py-4 rounded-[50px] sm:rounded-[100px] shadow-2xl h-[90vh] sm:h-fit border-[#333333] border-1 sm:border-0">
          <Link
            href="/#work"
            onClick={() => {
              closeNavBar();
          ...
          <Link
            ...
            className="py-[10px] px-[16px] button-gradient text-white rounded-[30px] flex items-center justify-center gap-2 hover:scale-[1.05] transition-all duration-300"
          >
            Start Project{" "}
            <svg
              width="20"
